...
\newcommand{\userfontpath}{C:/Users/marcus.wassmer/AppData/Local/Microsoft/Windows/Fonts/}
...
% Use smaller font in tables so wide tables (e.g. precedence table) fit
\usepackage{etoolbox}
% Reduce font and cell padding in tables to prevent wide tables from overflowing.
% Some tables (precedence, effects) have 5-6 columns with inline code.
% The widest table (operator precedence, 6 cols) extends ~36pt into the outer
% margin at \scriptsize — this stays within the physical page and won't be
% cut at the trim line, but the printer may flag it.
...
%% ============================================================
%% PDF BOX METADATA (TrimBox and BleedBox for print imposition)
%% ============================================================
% TrimBox: 0.125" (9pt) inset on all sides = 7" x 10" final trim
% BleedBox: full media = 7.25" x 10.25"
\newcommand{\setpdfboxes}{%
  \special{pdf:put @thispage << /TrimBox [9 9 513 729] /BleedBox [0 0 522 738] >>}%
}
\AtBeginShipout{\setpdfboxes}
\AtBeginShipoutFirst{\setpdfboxes}
\usepackage{enumitem}
\usepackage{calc}
...
% Prevent body text overflow: allow LaTeX to stretch inter-word spaces
% rather than letting long inline code push text past the margin
\emergencystretch=5em
\tolerance=400

% Allow breaks inside inline \texttt{} so long identifiers and specifier
% chains don't force body paragraphs to overflow. We redefine \texttt to
% use \ttfamily in horizontal mode (allowing LaTeX to break the line
% around the monospace text) rather than wrapping in an unbreakable \hbox.
\DeclareRobustCommand{\texttt}[1]{{\ttfamily #1}}
...
\vspace{0.5in}
{\Large\itshape The Verse Programming Language}\par
\vspace{0.5in}
{\large by Tim Sweeney}\par
\vspace{0.15in}
{\large and}\par
\vspace{0.15in}
{\large the Epic Games Verse Team}
\vfill
\end{titlepage}
...
% Pad with blank pages so total page count is divisible by 16
% (required for 16-page signature binding).
% On the first pass, totpages won't be known. On the second pass,
% we read the .aux file's page count and add the right number of blanks.
% For now, use a simple fixed approach: add 16 blank pages (enough to
% guarantee we overshoot), then on the final build, trim to exact count.
% Current content is ~479 pages. Next multiple of 16 = 480 or 496.
% 1 blank page to pad to 480 total (30 x 16 signatures)
